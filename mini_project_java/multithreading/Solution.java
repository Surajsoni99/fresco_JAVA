package mini_project_java.multithreading;
...
class Task1 extends Thread {
    ...
    public void run() {
        for (int j = beg; j < beg + a; j++) {
            //int randomIndex = (int) (Math.random() * (beg + a +1));
...
class Task2 extends Thread {
    ...
    public static int beg; //80

    @Override
    public void run() {
        for (int j = beg; j < beg + a; j++) {
            //int randomIndex = (int) (Math.random() * (a - beg + 1)) + beg;
            Solution.threadArray[j] = j;
...
class Task3 extends Thread {
    ...
    public static int beg;  //210

    @Override
    public void run() {
        for (int j = beg; j < Solution.threadArray.length; j++) {
            Solution.threadArray[j] = j;
